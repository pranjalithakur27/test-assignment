...
function verifyAssetData(org, resultBuffer, expectedId, product, ownerUserId, appraisedValue) {
    ...
    if (asset.product !== product) {
        doFail(`asset ${asset.assetID} has product of ${asset.product}, expected value ${product}`);
    }
...
async function main() {
    try {
        ...
        try {
            ...
            let asset = `asset${randomNumber + 1}`;
            const assetType = 'ValuableAsset';
            let result;
            let asset1Data = { objectType: assetType, assetID: assetID1, product: 'Engine', appraisedValue: 100 };
            let asset2Data = { objectType: assetType, assetID: assetID2, product: 'Fibre',  appraisedValue: 727 };
            ...
            console.log(`<-- result: ${prettyJSONString(result.toString())}`);
            verifyAssetData(mspOrg2, result, assetID1, 'Engine', 20, Org1UserId);
            ...
            console.log(`<-- result: ${prettyJSONString(result.toString())}`);
            verifyAssetData(mspOrg1, result, assetID1, 'Engine', 20, Org2UserId);
            ...
            console.log(`<-- result: ${prettyJSONString(result.toString())}`);
            verifyAssetData(mspOrg1, result, assetID2, 'Fibre', 35, Org1UserId);
